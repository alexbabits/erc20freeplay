...
contract FreePlayTokenTest is Test, EnumsEventsErrors {
    ...
    function test_initiateClaim_fulfillRandomWords() public {
        ...
        (,uint256 requestId,,uint64 requestedAt,,,,uint16 randomWord, State claimStatus,) = erc20FreePlay.getFreePlayPosition(1); // First Position ID is 1.
        assertEq(requestId, 0, "VRF request ID should not exist yet");
        assertEq(randomWord, 0, "should have no random word yet");
        assertEq(uint(claimStatus), uint(State.UNINITIALIZED), "should be integer 0");
        ...
        State claimStatusForClaim,) = erc20FreePlay.getFreePlayPosition(1); 
    ...
    function test_initiateMultipleClaims() public {
        ...
        (,uint256 requestId1,,uint64 requestedAt1,,,,,State claimStatus1,) = erc20FreePlay.getFreePlayPosition(1); 
        (,uint256 requestId2,,uint64 requestedAt2,,,,,State claimStatus2,) = erc20FreePlay.getFreePlayPosition(2); 
        (,uint256 requestId3,,uint64 requestedAt3,,,,,State claimStatus3,) = erc20FreePlay.getFreePlayPosition(3); 
    ...
    // Wrapping normal tokens into FP tokens with FP status not ON serves no purpose btw. Only for testing purposes.
    function test_wrapAndUnwrap_FP_OFF() public {
        ...
        // approve the exchange for wrapping and then wrap.
        vm.startPrank(ArbitraryYieldHandlingContract);
        erc20Mock.approve(address(exchange), amount);
        exchange.wrap(amount, Alice);

        assertEq(erc20FreePlay.balanceOf(Alice), amount, "Alice should have been forwarded the wrapped 20 FP tokens");
        assertEq(erc20Mock.balanceOf(address(exchange)), amount, "Exchange should have the 20 normal tokens");
        assertEq(erc20Mock.balanceOf(ArbitraryYieldHandlingContract), 0, "Yield should have been forwarded to Alice via FP tokens");
        assertEq(erc20FreePlay.balanceOf(ArbitraryYieldHandlingContract), 0, "Yield should have been forwarded to Alice via FP tokens");
        assertEq(erc20Mock.balanceOf(Alice), 0, "Alice should have 0 normal tokens");
        vm.stopPrank();

        vm.startPrank(Alice);
        // Approve the exchange for unwrapping FP tokens, then unwrap.
        erc20FreePlay.approve(address(exchange), amount);
        exchange.unwrap(amount);
        assertEq(erc20Mock.balanceOf(Alice), amount, "Alice should have 20 normal tokens now");
        assertEq(erc20FreePlay.balanceOf(Alice), 0, "Alice should have 0 FP tokens now");
        assertEq(erc20Mock.balanceOf(address(exchange)), 0, "Exchange should have 0 normal tokens left");    
        assertEq(erc20FreePlay.balanceOf(address(exchange)), 0, "Exchange should have 0 FP tokens");      
    }
    ...
    function test_wrapAndUnwrap_FP_ON() public {
        ...
        assertEq(erc20FreePlay.balanceOf(Alice), 0, "Alice should have been 0 FP tokens");
        assertEq(erc20Mock.balanceOf(Alice), 0, "Alice should have 0 normal tokens");
        assertEq(erc20Mock.balanceOf(ArbitraryYieldHandlingContract), 0, "Yield should have been forwarded to Alice via FP tokens");
        assertEq(erc20FreePlay.balanceOf(ArbitraryYieldHandlingContract), 0, "Yield should have been forwarded to Alice via FP tokens");
        assertEq(erc20Mock.balanceOf(address(exchange)), amount, "Exchange should have the 20 normal tokens");
        assertEq(erc20FreePlay.balanceOf(address(escrow)), amount, "Escrow should have the 20 FP tokens");
        (uint256 credits,,,,,,,,,) = erc20FreePlay.getFreePlayPosition(1); // First Position ID is 1.
        ...
        vm.startPrank(Alice);
        erc20FreePlay.initiateClaim(1); // Pos ID 1
        (,uint256 requestIdAlice,,,,,,,,) = erc20FreePlay.getFreePlayPosition(1); 
        coordinator.fulfillRandomWords(requestIdAlice, address(erc20FreePlay)); 
        (,,,,,,,uint16 randomWordAlice,,) = erc20FreePlay.getFreePlayPosition(1); 
        console.log("Random word generated for Alice's position: ", randomWordAlice);
        ...
        assertEq(erc20Mock.balanceOf(address(exchange)), 0, "Exchange should have 0 normal tokens left");    
        assertEq(erc20FreePlay.balanceOf(address(exchange)), 0, "Exchange should have 0 FP tokens");  
        assertEq(erc20FreePlay.balanceOf(address(escrow)), 0, "Escrow should have 0 FP tokens"); 
    ...
    function test_fixFailedRequestPosition() public {
        ...
        vm.startPrank(Alice);
        erc20FreePlay.initiateClaim(1); // First FP Position ID is 1.
        ...
        ) = erc20FreePlay.getFreePlayPosition(1); 

        vm.expectRevert();
        erc20FreePlay.fixFailedRequestPosition(1);
        ...
        // 1. Incorrect keyHash during request, 2. insufficient sub funds 3. fulfillRandomWords() itself reverts.
        ...
        ,,,
        randomWordForClaim, 
        claimStatusForClaim,) = erc20FreePlay.getFreePlayPosition(1); 
        
        assertEq(requestIdForClaim, 0, "VRF request ID should be reset to 0"); 
        ...
        erc20FreePlay.initiateClaim(1);
        (,uint256 requestId,,,,,,,,) = erc20FreePlay.getFreePlayPosition(1);
        coordinator.fulfillRandomWords(requestId, address(erc20FreePlay)); // 922 = randomWord
        erc20FreePlay.finalizeClaim(1, false);
        assertEq(erc20FreePlay.balanceOf(Alice), amount, "Alice should receive the underlying FP tokens");
    ...
    function test_fixFailedRequestPosition_Reverts() public {
        ...
        // Even though it's been over 1 day, since we have a random word for the position, cannot "fix" it bc it's not broken.
        vm.expectRevert();
        erc20FreePlay.fixFailedRequestPosition(1);
        vm.warp(block.timestamp + 93600);
        vm.expectRevert();
